// $Id: SimOpChannel.cxx,v 1.3 2010/03/26 20:08:36 brebel Exp $
///
/// \file  Simulation/SimOpChannel.cxx
///
///
...
namespace sim{
  ...
  // Default constructor
  //-------------------------------------------------
  SimOpChannel::SimOpChannel() 
    : iOpChannel(-1) //set an impossible channel number in the case where this is called without an opticalchannel.
                     //The reason for doing this is to follow the structure of SimChannel, which uses kBogusChannel
  {}

  //-------------------------------------------------
  SimOpChannel::SimOpChannel(int detNum)
    : iOpChannel(detNum)    
  {}

  //-------------------------------------------------
  void SimOpChannel::AddScintillationPhotons(TrackID_t     trackID,
  ...
  } // SimOpChannel::AddIonizationElectrons()


  //-------------------------------------------------
  double SimOpChannel::Photons(timePDclock_t iTimePDclock) const  //Number of photons
  {
    double numPhotons = 0.;
  ...
  double SimOpChannel::Energy(timePDclock_t iTimePDclock) const
  {
    double energy = 0.;
  ...
  std::vector<sim::SDP> SimOpChannel::TrackIDsAndPhotons(timePDclock_t startTimePDclock,
                                                        timePDclock_t endTimePDclock) const
  {
    // make a map of track ID values to sim::SDP objects

    if(startTimePDclock > endTimePDclock ){
      mf::LogWarning("SimOpChannel") << "requested TimePDclock range is bogus: "
  ...
  //-----------------------------------------------------------------------
  // the start and end iTimePDclock values are assumed to be inclusive
  std::vector<sim::TrackSDP>  SimOpChannel::TrackSDPs(timePDclock_t startTimePDclock,
                                                    timePDclock_t endTimePDclock) const
  {
    ...
    if(startTimePDclock > endTimePDclock ){
      mf::LogWarning("SimOpChannel::TrackSDPs") << "requested iTimePDclock range is bogus: "
					      << startTimePDclock << " " << endTimePDclock
					      << " return empty vector";
      return trackSDPs;
    }
    ...
    std::vector<sim::SDP> const sdps = TrackIDsAndPhotons(startTimePDclock, endTimePDclock);
  ...
  std::pair<SimOpChannel::TrackID_t,SimOpChannel::TrackID_t>
  SimOpChannel::MergeSimOpChannel(SimOpChannel const& channel,
                              int               offset)
  {
    if( this->Channel() != channel.Channel() )
      throw std::runtime_error("ERROR SimOpChannel Merge: Trying to merge different channels!");
    ...
    for(auto const& itr : channel.timePDclockSDPsMap()){
      ...
      // find the entry from this SimOpChannel corresponding to the iTimePDclock from the other
  ...
  //-------------------------------------------------
  struct SimOpChannel::CompareByTimePDclock {
  ...
  SimOpChannel::timePDclockSDPs_t::iterator SimOpChannel::findClosestTimePDclockSDP(storedTimePDclock_t iTimePDclock)
  {
    return std::lower_bound
      (timePDclockSDPs.begin(), timePDclockSDPs.end(), iTimePDclock, CompareByTimePDclock());
  }
  
  SimOpChannel::timePDclockSDPs_t::const_iterator SimOpChannel::findClosestTimePDclockSDP
