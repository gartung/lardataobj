 ...
 * 
 * Changes:
 * 20190208 Tracy Usher 02/08/2019
 *   Boldly stolen from Gianluca's "Wire.h" object
 *
 * ****************************************************************************/
...
namespace sim {
   ...
   * creating the Waveform object. In the simple approach, the whole readout window
   ...
   *     
   *     for (float ADCcount: Waveform.Signal()) ...
   *     
   * or they can analyze region by region:
   *     
   *     for (auto iROI = Waveform.begin_range(); Waveform.end_range(); ++iROI) {
   ...
   * should be stored together with sim::Waveform by the producer in a art::Assns
   * data product.
   */
  class Waveform {
    public:
      ...
      /// Default constructor: a Waveform with no signal information
      Waveform();
      
    private:
      raw::ChannelID_t    fChannel;   ///< ID of the associated channel
      geo::View_t         fView;      ///< View corresponding to the plane of this Waveform
      RegionsOfInterest_t fSignalROI; ///< Signal on the channel

    
    friend class WaveformCreator; // helper to create Waveforms in art
    ...
    public:
       ...
       * 
       * Signal is copied into the Waveform object. If possible, use the other
      ...
      bool operator< (const Waveform& than) const;
...
inline const sim::Waveform::RegionsOfInterest_t& sim::Waveform::SignalROI()                      const { return fSignalROI;        }
inline size_t                                    sim::Waveform::NSignal()                        const { return fSignalROI.size(); }
inline geo::View_t                               sim::Waveform::View()                           const { return fView;             }
inline raw::ChannelID_t                          sim::Waveform::Channel()                        const { return fChannel;          }
inline bool                                      sim::Waveform::operator< (const Waveform& than) const { return Channel() < than.Channel(); }
